...
--------------------------------
* Click on the OpenTA Link in Canvas or Moodle, (Canvas and Moodle
  referred to as `LTI` in this documentation).
* If you want to get rid of the Canvas frame

   * Click on ``Frameless OpenTA`` `after` you have opened logged
     in on OpenTA Canvas link

* If you want to access OpenTA outside of Canvas

   - Login via Canvas
   - Click on the key icon in the header and assign yourself a
     password
   - Then access the site directly via the URL and login you see in
     ``Frameless OpenTA``
...
* Access denied  : `Please enable cookies (and third-party cookies) to access OpenTA`
   ...
   - If you don't want to allow all third party cookies, allow those
     from ``[*.]instructure.com``

* Update your browswer; OpenTA enforces security settings that are
  violated in earlier browswers.
...
Several methods; the examiner has chosen one and it should be obvious
from the OpenTA page. All methods require a valid email from which you
will need to retrive an email from opentaproject@gmail.com.
...
* Anonymous login  method
   ...
   - After you login, you can and follow the instructions.
     ...
     .. note::
        In order to keep the bots away, you may need to answer
        a few exercises correctly before you will be allowed to
        register.
